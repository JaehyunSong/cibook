...
::: {layout-ncol=2}
![我々が考えている電子投票](figs/did/eVote1.jpg){fig-align="center" height=400px}

![現実](figs/did/eVote2.jpg){fig-align="center" height=400px}
:::
...
eVote |> 
  ...
  annotate("text", x = "伏見", y = 39.0, label = "東山区を除く平均値：39.0",
           hjust = 0.9, vjust = -0.25, size = 4) +
  labs(x = "行政区", y = "投票率（%）", caption = "注：今後の議論を単純化するために上京区はデータから除外した。") +
...
|処置（$D_i = 1$）|[$\mathbb{E}[Y_{i, \textsf{pre}}^1|D_i = 1]$]{.hl_blue}|[$\mathbb{E}[Y_{i, \textsf{post}}^1|D_i = 1]$]{.hl_red}|
...
- 私たちにできる計算：[$\mathbb{E}[Y_{i, \textsf{post}}^1|D_i = 1]$]{.hl_red} - [$\mathbb{E}[Y_{i, \textsf{pre}}^1|D_i = 1]$]{.hl_blue}
   - [2004年の東山区の投票率]{.hl_red} - [2000年の東山区の投票率]{.hl_blue}
- ATTが推定可能となる仮定：$\mathbb{E}[Y_{i, \textsf{post}}^0|D_i = 1]$ = $\mathbb{E}[Y_{i, \textsf{pre}}^1|D_i = 1]$
   - 2004年の北区の投票率 = 2000年の東山区の投票率

## 2つの推定方法（2）

||処置前（Pre）|処置後（Post）|
|:---:|:---:|:---:|
|統制（$D_i = 0$）|$\mathbb{E}[Y_{i, \textsf{pre}}^0|D_i = 0]$|[$\mathbb{E}[Y_{i, \textsf{post}}^0|D_i = 0]$]{.hl_blue}|
|処置（$D_i = 1$）|$\mathbb{E}[Y_{i, \textsf{pre}}^1|D_i = 1]$|[$\mathbb{E}[Y_{i, \textsf{post}}^1|D_i = 1]$]{.hl_red}|
...
- 私たちにできる計算：[$\mathbb{E}[Y_{i, \textsf{post}}^1|D_i = 1]$]{.hl_red} - [$\mathbb{E}[Y_{i, \textsf{post}}^0|D_i = 0]$]{.hl_blue}
   - [2004年の東山区の投票率]{.hl_red} - [2004年の北区の投票率]{.hl_blue}
- ATTが推定可能となる仮定：$\mathbb{E}[Y_{i, \textsf{post}}^0|D_i = 1]$ = $\mathbb{E}[Y_{i, \textsf{post}}^0|D_i = 0]$
   - 電子投票を導入しなかった場合の2004年の東山区の投票率 = 2004年の北区の投票率
...
$$
\begin{align}
\delta_{\textsf{ATT}} & = (Y_{\textsf{treat}, \textsf{post}} - Y_{\textsf{treat}, \textsf{pre}}) - (Y_{\textsf{control}, \textsf{post}} - Y_{\textsf{control}, \textsf{pre}}) \\
...
- 時間を通じた結果変数の変化の差を、処置群と統制群の間で比較
   ...
   - ある時点における処置群と統制群の差異のみを分析（$\times$）
- 推定量はATEでなく、主に[ATT]{.strong}
...
[**平均トレンドの仮定**]{.alert}[（parallel trend assumption）]{.small}
...
- （A）：処置群がもし処置を受けなかった場合の変化量（反事実 = 観察不可能）
- （B）：統制群における変化量（事実 = 観察可能）
- $\Rightarrow$ 「統制群の変化量」=「処置群が処置を受けなかった場合の変化量」
...
- 人為的な解釈が必要であり、並行トレンドの仮定に対する示唆的根拠[（*suggestive evidence*）]{.small}に過ぎない
...
- 処置群の差分：$\widehat{y}_{it}(D_i = 1, P_t = 1) − \widehat{y}_{it}(D_i = 1, P_t = 0)$
   - $\delta + \gamma = \underbrace{(\alpha + \delta + \beta + \gamma)}_{\widehat{y}_{it}(D_i = 1, P_t = 0)} - \underbrace{(\alpha + \beta)}_{\widehat{y}_{it}(D_i = 1, P_t = 0)}$
- 統制群の差分：$\widehat{y}_{it}(D_i = 0, P_t = 1) − \widehat{y}_{it}(D_0 = 0, P_t = 0)$
   - $\gamma = \underbrace{(\alpha + \gamma)}_{\widehat{y}_{it}(D_i = 0, P_t = 1)} - \underbrace{(\alpha)}_{\widehat{y}_{it}(D_i = 0, P_t = 0)}$
...
- 個体と時間の固定効果モデル
   - $\gamma_i$：個体のダミー変数
   - $\lambda_t$：時間のダミー変数
- $\delta$：処置効果[（ATT）]{.small}
   - 全処置群において処置を受けるタイミングが同じ場合のみ
   - 処置のタイミングが異なる場合、$\delta$はATEでも、ATTでもない
- 注意）3期以上のデータの場合、系列相関により標準誤差が過小評価される [[@bertrand-et-al-2004]]{.small}
...
- 例）2004年の東山区の投票率は、2000年の東山区の投票率と強い相関関係があり、2000年の東山区の投票率は、1996年の東山区の投票率と強い相関関係があり、...
   ...
   - 似たような観測が多くなるため、有効サンプルサイズ[（effective sample size）]{.small}は実際よりも小さい
- 系列相関が存在する場合、従来の標準誤差[（`lm()`等）]{.small}は過小評価される
   - $\Rightarrow$ [**個体でクラスター化した頑健な標準誤差**]{.alert}[（cluster-robust standard error）]{.small}
   - その他の方法）ブロック・ブートストラップ / データを処置前後の2期に集約
...
eVote |> 
  ...
  ggplot(aes(x = `1993`, y = `1996`)) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_text(aes(label = Ward_J)) +
  labs(x = "1993年の投票率", y = "1996年の投票率") +
  theme_jay()
...
- 時間と共に変化する交絡要因[（time-varying confounder）]{.small}の投入
...
- 個体 $\times$ 時間 $\times$ 処置有無の可視化[（以下は{panelView}パッケージを使用した例）]{.small}
...
1. **Step 1**：処置群が各時点において処置を受けたとするダミー変数を作成 [[例]{.button}](#sec-event-study-data)
   - 統制群の場合、すべて0
...
## イベント・スタディのためのデータ {#sec-event-study-data}
...
- プラセボのタイプ [[@gruber-hungerman-2008]]{.small}
   - タイミングをずらす：処置前の時期に効果が見られないか
   - 対象をずらす：本来は影響がないはずのグループに効果が見られないか
   - タイミングと対象を組み合わせ
- プラセボテストの回数
   - 1回：任意のプラセボで実施
   - 複数回：様々なプラセボを作成し、実施 [[@bertrand-et-al-2004]]{.small}
- 因果推論におけるプラセボテスト全般に関しては @eggers-et-al-2024 を参照
...
## タイミング $\times$ 対象のプラセボテスト

1. **Step1**：実際の処置群をデータから除外し、処置前のデータのみ抽出する
1. **Step2**：任意の対象が任意の時期に処置が行われたと仮定し、再コーディングする
   - 例）**北区が2000年**に電子投票を導入したかのように修正する（実際は東山区 $\times$ 2004年）
1. **Step3**：DID推定
1. **Step4**：処置効果が統計的有意に0でない場合、並行トレンドが満たされていないと判断

```{r}
placebo_fit4 <- eVote |> 
  filter(year <= 2000 & WardID != 5 & WardID != 2) |> 
  mutate(eVote = if_else(year == 2000 & Ward_J == "北区", 1, 0)) |> 
  feols(turnout ~ eVote | Ward_J + year, data = _) 
...
- この状況下における伝統的なTWFEの推定値は**深刻なバイアス**を含む [[@goodman-bacon-2021]]{.small}
   - この場合、TWFEの推定値はATEでも、ATTでもない[^twfe-att]
- 段階的導入への対処
   - Sun & Abraham補正 [[@sun-abraham-2021]]{.small}
   - 反事実予測アプローチ [[@liu-wang-xu-2024]]{.small}^[処置が0に戻るような反転可能な処置（reversible treatments / switching treatments）でも使用可能]など

[^twfe-att]: 処置が一斉に導入され、反転されない（処置の撤回がない）場合、TWFEの推定値はATTとして解釈できる。また、処置群と統制群における処置効果が等しいと仮定出来る場合、ATEとして解釈も可能だが、これは確認することも出来ない非常に強い仮定である。

## 段階的導入の例

- 例）女性参政権
   - WY（1869） $\rightarrow$ CO（1893） $\rightarrow$ IL（1913） $\rightarrow$ 全米（1920）
- 娯楽用マリファナの合法化
...
処置の反転[（treatment reversal）]{.small}：処置状態が0 $\rightarrow$ 1 $\rightarrow$ 0のように変化するケース

- 例）京都府の電子投票：東山区（2004〜2012年）、上京区（2008〜2012年）
   - 段階的導入 & 処置反転の例
...
df_reverse <- expand.grid(unit = 1:n_units, time = 1:n_periods) |> 
  mutate(
    type = "Reversible",
    D = case_when(
      unit == 1 & time %in% 3:6 ~ 1,
      unit == 2 & time >= 5 ~ 1,
      unit == 3 & time %in% 6:7 ~ 1,
      TRUE ~ 0
    )
  )

df_all <- bind_rows(df_normal, df_reverse) |> 
  mutate(type = factor(type, levels = c("Single Timing", "Reversible")))
...
[**Synthetic Control Method**]{.strong}

- 合成コントロール法、または合成制御法と訳される（[SCM]{.strong}ともよく呼ばれる）
- 統制群を合成することで処置群の潜在的結果を作り上げる手法
   - 処置群と並行トレンドを満たす架空の統制群を作り上げる
   - @abadie-alexis-hainmueller-2010 と @abadie-alexis-hainmueller-2015 によって提案され、 @xu-2017 が一般化SCM[（generalized synthetic control method）]{.small}を提案
- Rの{synth}および{fect}[^gsynth]パッケージで実装可能

[^gsynth]: {gsynth}というパッケージもあるが、今は{fect}と統合されている。2026年8月現在、CRANから入手可能な{gsynth}は{fect}のラッパー（wrapper）パッケージだ。
