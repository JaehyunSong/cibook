...
- 合理的有権者の投票参加モデル[@riker-ordeshook-1968]
...
#| fig-width: 8
#| fig-height: 4
read_csv("_data_slide/rdd/ElectoralSystem_Turnout.csv") |> 
...
- コミューン[^commune]（*commune*）議会選挙（1982〜2013[^reform]）
   ...
   - 以下は架空のデータ（実際の研究成果は @eggers-2015 等を参照）
...
## 感度分析[（sensitivity analysis）]{.small}

RDDの推定結果が、分析方法を少し変えてもどの程度安定しているか

- バンド幅や関数形、カーネル関数など若干の変更が分析結果を大きく変える場合、今回得られた結果も<ruby>たまたま<rt>・・・・</rt></ruby>のものである可能性が高い
- 感度分析の例
   - **バンド幅**（ほぼ必須; 最適バンド幅の1/2、2倍が定番）
   - **多項式の次数** 
   - **カーネル関数**
   - **共変量の有無**
   - **ドーナツRDD**（閾値周辺のケースを除外）

## プラセボ検定[（placebo test）]{.small}
...
中断時系列デザイン（Interrupted Time Series; ITS）

## 地理的回帰不連続デザイン

地理的回帰不連続デザイン（Geographic RDD; GRD）
