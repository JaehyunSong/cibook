...
- 例1：我が社[（カナダ国鉄）]{.small}の株価を上げたい
   - **解決策** $\Rightarrow$ アメリカ人に新鮮な果物を無料で提供する
- 例2：我が国[（アメリカ）]{.small}の肥満率を下げるために、国民に果物をよりたくさん摂取してもらいたい
   - **解決策** $\Rightarrow$ カナダ国鉄の株を爆買いする

#### 政治学者とスーパーマーケット

- 例：我が社[（Whole Foods）]{.small}に対する顧客満足度を高めたい
...
> *"Conditioning is the soul of statistics"* [[@blitzstein-hwang-2019, p. 46]]{.small}
...
[個体レベルにおける処置効果]{.alert}[（individual treatment effect; [ITE]{.strong}）]{.small}
...
[平均処置効果]{.alert}[（average treatment effect; [ATE]{.strong}）]{.small}：[ITE]{.hl_blue}の平均

$$
\textcolor{red}{\textsf{ATE}} = \frac{1}{n} \sum_i^n \bigl( \textcolor{royalblue}{Y_i (1) - Y_i (0)} \bigr) = \mathbb{E}[Y_i(1)] - \mathbb{E}[Y_i(0)]
$$

|$i$|$D_i$|$Y_i(0)$|$Y_i(1)$|$\textsf{ITE}_i$|
|---:|---:|------:|------:|------:|
|1|0|344|444|**ITE**<sub>1</sub> = &nbsp;[100]{.hl_red}|
|2|0|455|505|**ITE**<sub>2</sub> = &nbsp;[50]{.hl_red}|
|3|0|479|379|**ITE**<sub>3</sub> = &nbsp;[-100]{.hl_red}|
|4|1|404|554|**ITE**<sub>4</sub> = &nbsp;[150]{.hl_red}|
|5|1|295|395|**ITE**<sub>5</sub> = &nbsp;[100]{.hl_red}|
|6|1|248|248|**ITE**<sub>6</sub> = &nbsp;[0]{.hl_red}|
|平均||370.8|420.8|[50]{.strong}|

## 平均処置効果（2）

[平均処置効果]{.alert}[（average treatment effect; [ATE]{.strong}）]{.small}：[$Y_i(1)$の平均値 - $Y_i(0)$の平均値]{.hl_blue}

$$
\textcolor{red}{\textsf{ATE}} = \frac{1}{n} \sum_i^n \bigl( Y_i (1) - Y_i (0) \bigr) = \textcolor{royalblue}{\mathbb{E}[Y_i(1)] - \mathbb{E}[Y_i(0)]}
$$
...
||$Y_i(0)$|$Y_i(1)$|
|:---|:---:|:---:|
|$D_i = 0$|1. [$Y_i(0|D_i = 0)$]{.hl_blue}|2. [$Y_i(1|D_i = 0)$]{.hl_red}|
|$D_i = 1$|3. [$Y_i(0|D_i = 1)$]{.hl_red}|4. [$Y_i(1|D_i = 1)$]{.hl_blue}|

1. $Y_i(0 | D_i = 0)$：統制群の個体$i$が処置を受けなかった場合の$Y_i$ $\rightarrow$ [**事実**]{.hl_blue}
   - 統制群における$Y_i(0)$は観察可能 $\leadsto$ $\mathbb{E}[Y_i(0 | D_i = 0)]$も計算可能
1. $Y_i(1 | D_i = 0)$：統制群の個体$i$が処置を受けた場合の$Y_i$ $\rightarrow$ [**反事実**]{.hl_red}
   - 統制群における$Y_i(1)$は観察不可能 $\leadsto$ $\mathbb{E}[Y_i(1 | D_i = 0)]$も計算不可能
1. $Y_i(0 | D_i = 1)$：処置群の個体$i$が処置を受けなかった場合の$Y_i$ $\rightarrow$ [**反事実**]{.hl_red}
   - 処置群における$Y_i(0)$は観察不可能 $\leadsto$ $\mathbb{E}[Y_i(0 | D_i = 1)]$も計算不可能
1. $Y_i(1 | D_i = 1)$：処置群の個体$i$が処置を受けた場合の$Y_i$ $\rightarrow$ [**事実**]{.hl_blue}
   - 処置群における$Y_i(1)$は観察可能 $\leadsto$ $\mathbb{E}[Y_i(1 | D_i = 1)]$も計算可能

## 因果推論の根本問題とATE

[観測値の平均値の差分]{.alert}は計算可能

- $\mathbb{E}[Y_i(1)] - \mathbb{E}[Y_i(0)]$でなく、$\mathbb{E}[Y_i(1 | D_i = 1)] - \mathbb{E}[Y_i(0 | D_i = 0)]$は計算可能
   - これはATE? $\Rightarrow$ Maybe not...
